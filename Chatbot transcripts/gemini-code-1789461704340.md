...
1. **Semantic Parsing & Schema Mapping:** The NLU layer maps the query to the `INVENTORY_RESTOCK_PRIORITIZATION` intent class, binding parameters to four core metrics: Back-in-Stock (BIS) subscriptions, wishlist velocity (7-day change), variant sellout rates[cite: 1, 3], and occasion tag proximity weights[cite: 1].
2. **Deterministic Calculation (Non-LLM Engine):** The engine executes a mathematical model over the merchant's first-party graph to generate a **Restock Priority Score (RPS)** and estimate at-risk revenue[cite: 1]:
...
$$\text{At-Risk Revenue} = \sum (\text{Unmet BIS Requests} + \text{Saved Carts}) \times \text{Unit Price} \quad \text{[cite: 1]}$$

3. **Governed JSON Output:** The engine produces a verified JSON payload containing exact SKU ranks, calculated risk values, and actionable recommendations[cite: 1, 3].
4. **Narrative Synthesis & Workflow Triggers:** The LLM formats the payload into a merchant-facing summary equipped with operational triggers (e.g., *"Draft Purchase Order"* or *"Sync to Klaviyo Restock Flow"*[cite: 4]).
...
* **Stale Data Race Conditions:** Recommending restocks for items replenished minutes prior.
  * *Safeguard:* Enforce a real-time verification ping against Shopify's `inventory_levels/update` webhook before rendering responses, capping inventory cache TTL at 60 seconds[cite: 4].
...
* **Temporal Demand Distortion:** Recommending restocks for out-of-season products due to stale historical wishlist data.
  * *Safeguard:* Apply exponential decay ($e^{-\lambda t}$) to intent events older than 60 days, discounting non-recurring intent unless aligned with upcoming calendar facets (e.g., Diwali, Christmas)[cite: 1].
...
| :--- | :--- | :--- |
| **1. Pre-Purchase Intent Graph** | Captures pre-purchase intent (wishlists, OOS hits) before an order exists[cite: 1]. | Platform analytics only track completed orders; Swym captures uncaptured shopper demand across 45,000+ stores[cite: 1]. |
...
| **3. Operational Ecosystem Sync** | Direct API triggers to ERPs (NetSuite) & ESPs (Klaviyo)[cite: 4]. | Automates restock workflows directly in external execution systems[cite: 4]. |
